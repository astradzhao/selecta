...
import { submissionProposalStatusEnum, type SubmissionProposalStatus } from "@selecta/db";
...
describe("PROPOSAL_STATUS", () => {
  it("defines a label and tone for every schema proposal status", () => {
    for (const status of submissionProposalStatusEnum.enumValues) {
  ...
  it("does not map statuses that are not in the schema enum", () => {
    ...
    const fromSchema = [...submissionProposalStatusEnum.enumValues].sort();
    assert.deepEqual(mapped, fromSchema);
  });

  it("treats committed as success and needs_review as warning, not failure", () => {
    assert.equal(proposalStatus("committed" satisfies SubmissionProposalStatus).tone, "success");
