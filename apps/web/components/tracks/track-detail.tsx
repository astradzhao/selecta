...
import { ApiClientError, getSong, type ApiSong } from "@/lib/api";
...
export function SongDetail({ songId }: { songId: string }) {
  ...
  useEffect(() => {
    ...
    startLoad(async () => {
      try {
        ...
        if (cancelled) return;
        setSong(null);
        setError(err instanceof ApiClientError ? err.message : "Failed to load song.");
      }
    });
    return () => {
      cancelled = true;
  ...
  if (pending && !song) {
    return <p className="text-muted-foreground text-sm">Loading song…</p>;
  }
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-6 sm:flex-row">
        <div className="bg-muted relative h-48 w-48 shrink-0 overflow-hidden rounded-2xl">
          {song.artworkUrl ? (
            <Image src={song.artworkUrl} alt="" fill className="object-cover" sizes="192px" />
          ) : null}
        </div>
        <div className="min-w-0 flex-1 space-y-4">
          <div className="space-y-2">
            <p className="text-muted-foreground text-xs tracking-[0.16em] uppercase">Song</p>
            <h1 className="text-3xl font-semibold tracking-tight text-balance">{song.title}</h1>
      ...
      <dl className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <dd className="mt-1 text-sm">
            {song.genres.length ? song.genres.map((genre) => genre.name).join(", ") : "None"}
          </dd>
        </div>
        <div>
          <dt className="text-muted-foreground text-xs tracking-wide uppercase">Subgenres</dt>
          <dd className="mt-1 text-sm">
            {song.subgenres.length ? song.subgenres.map((item) => item.name).join(", ") : "None"}
          </dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="mt-1 text-sm">
            Outbound: {song.hasOutboundTransitions ? "yes" : "no"} · Inbound:{" "}
            {song.hasInboundTransitions ? "yes" : "no"}
          </dd>
        </div>
      </dl>
    </div>
