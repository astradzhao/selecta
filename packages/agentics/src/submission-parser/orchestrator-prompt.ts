    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          "- Phrases like `at bar 33`, `bar 18`, `from bar 16`, `bars 32-48`, `cue A`, `hot cue B`, technique/intent/quality words are NOT part of the search query.",
          "- Put left-side bar on `transition.fromBar`, right-side bar on `transition.toBar`.",
          "- Put left-side hot cue on `transition.fromCue`, right-side hot cue on `transition.toCue` (Pioneer A–H or Serato 1–8). Bars and cues may both be set.",
          ...
          '- `echo chainsmokers cue A bar 33 -> sweet nothing calvin harris cue B bar 1` → m1.mention="echo chainsmokers"; m2.mention="sweet nothing calvin harris"; fromCue="A"; fromBar=33; toCue="B"; toBar=1; bidirectional=false',
          '- `animals martin garrix hot cue C -> levels avicii hot cue 1` → m1.mention="animals martin garrix"; m2.mention="levels avicii"; fromCue="C"; toCue="1"; fromBar=null; toBar=null; bidirectional=false',
          "",
          "Hard anti-patterns:",
          "- Do NOT invent titleHint/artistHint splits.",
          "- Do NOT emit 3–4 mentions.",
          "- Do NOT drop title/artist tokens from either side.",
          "- Do NOT leave `at bar N` / `bar N` / `cue A` / `hot cue B` inside `mention`.",
          "- Do NOT put bar numbers or hot cues only in `mention` and leave fromBar/toBar/fromCue/toCue null when the span states them.",
      ...
      },
      {
        ...
        body: [
          ...
          "- Never invent bars, hot cues, techniques, or intents not implied by the span.",
          "- When bars or hot cues ARE stated, set fromBar/toBar/fromCue/toCue — do not bury them in mention text.",
...
export function buildSingleTransitionUserPrompt(sourceText: string): string {
  return [
    ...
    "Emit exactly two mentions (m1, m2). Each mention is a Spotify search query (title + artist tokens only) — strip bar/hot-cue/technique cues into transition.fromBar/toBar/fromCue/toCue (and other transition fields). Set titleHint and artistHint to null.",
    "",
    sourceText.trim(),
  ].join("\n");
}
